var MEM_SIZE = 512, MAX_ADDRESS = MEM_SIZE - 1, BIT_MASK_16 = 65535, BIT_MASK_SIGN = 32768, DECIMAL_LENGTH = 10, HEX_LENGTH = 16, MAX_POSITIVE = BIT_MASK_16 >> 1, MIN_NEGATIVE = BIT_MASK_SIGN, MAX_HEX_ADDRESS_LENGTH = MAX_ADDRESS.toString(HEX_LENGTH).length, MAX_ADDRESS_LENGTH = MAX_ADDRESS.toString().length, MAX_16_BIT_HEX_DIGITS = 4, MAX_16_BIT_DEC_DIGITS = 5, PC_TRACKING_COLOR = "pink", SP_TRACKING_COLOR = "lightgreen", HALF_TABLE_LENGTH = 186, COMMENT = ";", LABEL_INDICATOR = ".", INS_TYPE_MEM_ACCESS = "memory_access", INS_TYPE_LOGICAL = "logical", INS_TYPE_ARITHMETIC =
    ...
    MOV: {
        ...
        INS_PC: "+3",
        INS_SP: "+0",
        INS_DESCRIPTION: "Sets memory location specified by arg1 to the value in arg0. If arg0 is a memory value it takes the value at arg0 and places it into the memory address specified in arg1."
    },
    POP: {
        N_ARGS: 1,
        ARG0: "R",
...
function generate_byte_code_mapping() {
    var a = [0], b;
    for (b in INSTRUCTIONS)if (INSTRUCTIONS.hasOwnProperty(b))for (var c = INSTRUCTIONS[b], d = c.ARG0, e = 0, f = d.length; e < f; e++)for (var g = c.ARG1, k = g.length, h = 0; h < k; h++)a.push([b, d[e] + g[h]]);
    return a
}
...
function do_cmp(a, b) {
    var c = get_arg_val(a), d = twos_invert_sign(get_arg_val(b));
    arithmetic_flag_setting(c, d, c + d)
}
function do_brn(a) {
    1 == getCCF("N") ? setPC(a) : setPC(getPC() + 2)
}
function do_bra(a) {
    setPC(a)
}
function do_brz(a) {
    1 == getCCF("Z") ? setPC(a) : setPC(getPC() + 2)
}
function do_brg(a) {
    1 == getCCF("N") || 1 == getCCF("Z") ? setPC(a) : setPC(getPC() + 2)
}
function do_jsr(a) {
    var b = getSP(), c = getPC();
    write_memory(b, c + 2);
...
function check_overflow(a, b, c) {
    return is_positive(a) && is_positive(b) ? !is_2s_positive(c) : is_negative(a) && is_negative(b) ? !is_2s_negative(c) : !1
}
function checkR(a) {
...
function checkI(a) {
    var b = String(a);
    return /(^\$0x[0-9a-f]+$)|(^\$[0-9]+$)/i.test(a) ? (a = parseInt(b.substring(1)), 0 <= a && a <= BIT_MASK_16) : 2 < b.length && "$." === b.substring(0, 2) ? (b = b.substring(1), b in LABELS2LINES || b in LABELS2MEM) : !1
}
...
function assemble() {
    ...
    for (var e = 1, f = 0; f < a.length; f++) {
        var g = strip_label_definition(strip_whitespace_and_comment(a[f]));
        if ("" !== g) {
            for (var k = g.split(","), g = 0; g < k.length; g++)k[g] = k[g].trim();
            var g = "", h;
            if (2 == k.length) {
                h = k[0].split(/\s+/g);
                var l = "", k = k[1];
                2 == h.length ? (g = h[0], l = h[1], h = check_instruction(g, l, k, 2), h.state ? (d[e] = c.length,
                    c.push(g), c.push(l), c.push(k)) : (b.push("Line " + e + " " + h.error), c = [])) : (b.push("Line " + e + " " + ERROR_INCORRECT_SPACING), c = [])
            } else 1 == k.length ? (l = k[0].split(/\s+/g), 1 == l.length ? (g = l[0], h = check_instruction(g, "", "", 0), h.state ? (d[e] = c.length, c.push(g)) : (b.push("Line " + e + " " + h.error), c = [])) : 2 == l.length ? (g = l[0], l = l[1], h = check_instruction(g, l, "", 1), h.state ? (d[e] = c.length, c.push(g), c.push(l)) : (b.push("Line " + e + " " + h.error), c = [])) : (b.push("Line " + e + " " + ERROR_INCORRECT_SPACING), c = [])) : (b.push("Line " + e + " " + ERROR_INCORRECT_ARGS),
                c = [])
        }
        e++
    }
    c.length > MEM_SIZE && (c = [], b.push("Line " + e + " " + ERROR_INSUFFICIENT_MEMORY));
    if (b.length) {
    ...
    a = c.length;
    for (f = 0; f < a; f++)b = c[f], "." === b[0] ? b = b in LABELS2MEM ? LABELS2MEM[b] : d[LABELS2LINES[b]] : "$" === b[0] && 2 < b.length && "." === b[1] && (b = b.substring(1), b = b in LABELS2MEM ? "$" + LABELS2MEM[b] : "$" + d[LABELS2LINES[b]]), write_memory(f, b);
    for (f = 0; f < a; f++)write_op_code(f);
    LINE2MEM = jQuery.extend(!0,
        {}, d);
    MEM2LINE = _.invert(d);
    for (var m in LABELS2LINES)if (LABELS2LINES.hasOwnProperty(m)) {
        d = LINE2MEM[LABELS2LINES[m]];
        if (!check_memory_limits(d)) {
            write_error_to_console(ERROR_ADDRESS_OUT_OF_BOUNDS);
            return
        }
        c = m.slice(1);
        $("#label" + d).html(c).attr("data-toggle", "tooltip").attr("title", c)
    }
    for (m in LABELS2MEM)LABELS2MEM.hasOwnProperty(m) && (c = m.slice(1), $("#label" + LABELS2MEM[m]).html(c).attr("data-toggle", "tooltip").attr("title", c));
...
function step() {
    ...
    if (b in INSTRUCTIONS && "STP" !== b)c = INSTRUCTIONS[b].N_ARGS, 0 == c ? INSTRUCTIONS[b].f() : 1 == c ? (c = get_memory(a + 1), INSTRUCTIONS[b].f(c)) : 2 == c && (c = get_memory(a + 1), a = get_memory(a + 2), INSTRUCTIONS[b].f(c, a)), a = getPC(), write_to_console("End step at address " + a); else if (c = parseInt(b), !isNaN(c) && BYTE_TO_INS.length > c && 0 < c) {
        ...
        c = d[0];
        "-" === c ? INSTRUCTIONS[b].f() : (c = set_op_code_type(get_op_code(a + 1), c), d = d[1], "-" === d ? INSTRUCTIONS[b].f(c) : (a =
            set_op_code_type(get_op_code(a + 2), d), INSTRUCTIONS[b].f(c, a)));
        a = getPC();
...
function execute_program() {
    PROGRAM_INTERVAL_ID = setInterval(function () {
        ...
        if (b in INSTRUCTIONS && "STP" !== b) {
            ...
            if (0 == c)INSTRUCTIONS[b].f(); else if (1 == c)c = get_memory(a + 1), INSTRUCTIONS[b].f(c); else if (2 == c) {
        ...
        } else c = parseInt(b), !isNaN(c) && BYTE_TO_INS.length >
        c && 0 < c ? (b = BYTE_TO_INS[c][0], d = BYTE_TO_INS[c][1], c = d[0], "-" === c ? (INSTRUCTIONS[b].f(), a += 1) : (c = set_op_code_type(get_op_code(a + 1), c), d = d[1], "-" === d ? (INSTRUCTIONS[b].f(c), a += 2) : (d = set_op_code_type(get_op_code(a + 2), d), INSTRUCTIONS[b].f(c, d), a += 3)), !IGNORE_BREAKPOINTS && pcAtBP(MEM2LINE, a) && (write_to_console("Breakpoint hit, main memory address: " + a), stop_program_running())) : (write_to_console("Finished running program."), stop_program_running())
    }, CLOCK_PERIOD)
}
function stop_program_running() {
    RUNNING && (enable_buttons_when_run(), clearInterval(PROGRAM_INTERVAL_ID), IGNORE_BREAKPOINTS = RUNNING = !1)
}
...
function jump2pc_in_mm() {
    var a = $("#address" + getPC()).parent().position();
    $("#div_main_memory").scrollTop(Math.max(0, a.top - HALF_TABLE_LENGTH))
}
function jump2sp_in_mm() {
    var a = $("#address" + getSP()).parent().position();
    $("#div_main_memory").scrollTop(Math.max(0, a.top - HALF_TABLE_LENGTH))
}
function change_clock_rate() {
    var a = parseInt($("#program_speed").val());
    CLOCK_PERIOD = isNaN(a) || 0 === a ? 0 : 1E3 / a;
    RUNNING && (stop_program_running(), resume_program_running())
}
...
var asm_grammar = {
    ...
    Lex: {
        ...
        decorator: ["RE::/[$]0x[0-9a-fA-F]+/", "RE::/[$][1-9]\\d*/", "RE::/[$]0(?![\\dx])/"],
        number: ["RE::/0x[0-9a-fA-F]+/", "RE::/[1-9]\\d*/", "RE::/0(?![\\dx])/"],
        Instruction: {autocomplete: !0, tokens: Object.keys(INSTRUCTIONS)},
        Register: {
            autocomplete: !0,
            tokens: Object.keys(LIST_REG_NAMES)
        }
    },
    Syntax: {asm: "comment | decorator | number | Label | Instruction | Register | identifier"},
    Parser: [["asm"]]
}, asm_mode = CodeMirrorGrammar.getMode(asm_grammar);
